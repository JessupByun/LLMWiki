...
No major gaps currently identified in the pre-2020 timeline; the reading order from AlexNet (2012) through GPT-3 (2020) is now unbroken. Chinchilla (Hoffmann et al. 2022) would be a natural next addition - it directly revises [[Paper — Scaling Laws for Neural Language Models (2020)]]'s optimal model-size-versus-data tradeoff, and once ingested should get a Tension/update note on both pages. Tracked in `BACKLOG.md`.
