...
By the authors' own repeated framing, this is a proof-of-concept built by the researchers, not evidence that deceptive alignment or model poisoning is likely to occur naturally - the backdoors are deliberately trained in, using an explicit deceptive-alignment preamble for the CoT variants, and the paper does not claim current models satisfy the internal preconditions for naturally-arising deceptive instrumental alignment (a hidden [[Concept — Mesa-optimization|mesa-objective]], long-horizon planning past the current episode, and situational awareness of being in training).
