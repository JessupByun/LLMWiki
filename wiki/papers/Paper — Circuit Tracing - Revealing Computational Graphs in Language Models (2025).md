...
sources: ["https://transformer-circuits.pub/2025/attribution-graphs/methods.html"]
...
A separate analysis studies the CLT's raw, prompt-independent "virtual weights" between features to recover general, prompt-independent circuit structure - demonstrated on an addition circuit, where features like a memorized-lookup-table component and a magnitude-estimate component show up as consistently connected regardless of which specific numbers are being added - correcting for a severe interference problem along the way: millions of features sharing one residual stream produce large spurious weights between features that never actually co-activate, addressed with a reweighting technique the paper calls TWERA.

## Results

The largest 18-layer research-model CLT (10 million features) reaches a striking sufficiency benchmark: replacing the real model's MLPs with CLT reconstructions still reproduces the underlying model's actual top-1 next-token prediction on about half of a diverse sample of pretraining-style prompts, given how aggressive a substitution that is.
